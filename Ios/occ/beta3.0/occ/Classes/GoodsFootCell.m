...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        UIImageView *titleImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, _cartButton.frame.origin.y+_cartButton.frame.size.height+10, 300, 20)];
        ...
        [_favoriteItemButton setFrame:CGRectMake(0, _logoImageView.frame.origin.y+_logoImageView.frame.size.height+20, 145, 40)];
        ...
        [_favoriteItemButton addTarget:self action:@selector(addItemToFavorite:) forControlEvents:UIControlEventTouchUpInside];
        [_favoriteItemButton setTitle:@"收藏商品" forState:UIControlStateNormal];
...
-(void)setData:(NSDictionary*)data
{
    ...
    if ([data objectForKey:@"shopProp"]!=nil)
    {
        [_typeLabel setText:[data objectForKey:@"shopProp"]];
    }
    else
    {
        [_typeLabel setText:@"未知"];
    }
    ...
    if ([data objectForKey:@"stat"])
    {
        int stat=[[data objectForKey:@"stat"]intValue];
        if (stat==1)
        {
        ...
        else if (stat==0)
        {
            ...
            [_cartButton setTitle:@"商品已下架" forState:UIControlStateNormal];
            [_cartButton setUserInteractionEnabled:NO];
            return;
        }
    }
    
    if ([data objectForKey:@"itemType"])
    {
        int itemType=[[data objectForKey:@"itemType"]intValue];
        if (itemType==0)
        {
            [_cartButton setTitleColor:COLOR_FFFFFF forState:UIControlStateNormal];
            [_cartButton setBackgroundImage:_redImage forState:UIControlStateNormal];
        ...
        else if (itemType==1)
        {
            [_cartButton setTitleColor:COLOR_000000 forState:UIControlStateNormal];
            [_cartButton setBackgroundImage:_grayImage forState:UIControlStateNormal];
            [_cartButton setTitle:@"展示商品" forState:UIControlStateNormal];
            [_cartButton setUserInteractionEnabled:NO];
        }
        else if (itemType==2)
        {
            [_cartButton setTitleColor:COLOR_000000 forState:UIControlStateNormal];
            [_cartButton setBackgroundImage:_grayImage forState:UIControlStateNormal];
            [_cartButton setTitle:@"虚拟商品" forState:UIControlStateNormal];
            [_cartButton setUserInteractionEnabled:NO];
        }
        else if (itemType==3)
        {
            [_cartButton setTitleColor:COLOR_000000 forState:UIControlStateNormal];
            [_cartButton setBackgroundImage:_grayImage forState:UIControlStateNormal];
            [_cartButton setTitle:@"即将推出" forState:UIControlStateNormal];
            [_cartButton setUserInteractionEnabled:NO];
        }
    }
}
