...
- (void)viewDidLoad
{
    ...
    [titleLable setText:[NSString stringWithFormat:@"新建收货地址"]];
...
- (void)viewDidUnload
{
    [self setCommunityToolBar:nil];
    [self setProvinceToolBar:nil];
    [super viewDidUnload];
}
...
- (void)doConfirm:(id)sender
{
    ...
    int addrCount=[[Singleton sharedInstance].addressList count];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0),
                   ^{
                       ...
                       if (root==nil){
                           ...
                           dispatch_async(dispatch_get_main_queue(),
                                          ^{
                                              [[NSNotificationCenter defaultCenter] postNotificationName:@"addAddressSuccessNotification" object:nil];
