...
#import "MyNotifyCell.h"

@interface MyNotifyListViewController ()<MJRefreshBaseViewDelegate>
{
...
@implementation MyNotifyListViewController
...
- (void)viewDidLoad
{
    ...
    [titleLable setText:[NSString stringWithFormat:@"通知"]];
...
//    UIView *rightButton = [CommonMethods navigationButtonWithTarget:self andSelector:@selector(doEdit:) andButtonType:OCCNavigationButtonTypeEdit andLeft:NO];
//    [headView addSubview:rightButton];
    ...
    OCCSegement *segement = [[OCCSegement alloc] initWithFrame:CGRectMake(10, (HEADER_HEIGHT - 28)/2, SCREEN_WIDTH - 10*2, 28)
                                                 ...
                                                 andTitleArray:[NSArray arrayWithObjects:@"全部",@"交易",@"系统",nil]];
...
- (void)selectedSegementIndex:(NSInteger)index
{
    switch (index) {
        case 0:
            ...
            break;
        case 1:
            self.type=NotifyTypeTrade;
            break;
        case 2:
            self.type=NotifyTypeSystem;
            break;
        default:
            break;
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    int height=[MyNotifyCell getCellHeight:data];
    return height;
}
...
{
    static NSString *MyNotifyCellIdentifier=@"MyNotifyCellIdentifier";
    
    MyNotifyCell *cell = (MyNotifyCell*)[tableView dequeueReusableCellWithIdentifier:MyNotifyCellIdentifier];
    if (cell == nil)
    {
        cell = [[MyNotifyCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:MyNotifyCellIdentifier];
    }
...
- (void)loadDataList
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0),
                   ^{
                       ...
                       ASIFormDataRequest *request=[CommonMethods asiHttpRequestWithURL:notifyListMobile_URL andData:reqdata andDelegate:nil];
                       ...
                       if (root==nil){
                           ...
                           dispatch_async(dispatch_get_main_queue(),
                                          ^{
                                              ...
                                              NSArray *dataList=[data objectForKey:@"notifyList"];
