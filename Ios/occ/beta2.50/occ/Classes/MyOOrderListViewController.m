...
@interface MyOOrderListViewController ()<MJRefreshBaseViewDelegate>
{
...
@implementation MyOOrderListViewController
...
- (void)viewDidLoad
{
    ...
    [titleLable setText:[NSString stringWithFormat:@"我的订单"]];
    ...
    switch (self.stat)
    {
        case MyOOrderStatWillSend:
        {
            [titleLable setText:@"待发货"];
        }
            break;
        case MyOOrderStatWillReceive:
        {
            [titleLable setText:@"待收货"];
        }
            break;
        case MyOOrderStatWillComplete:
        {
            [titleLable setText:@"已完成"];
        }
            break;
        case MyOOrderStatWillOther:
        default:
            break;
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *order=[_dataList objectAtIndex:section];
    NSArray *itemList=[order objectForKey:KEY_ITEMLIST];
    //int type=[[order objectForKey:@"orderType"]intValue];
    if (row==0)//显示订单时间
    {
        return 30.0;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (row==0)
    {
        ...
        MyOOrderStatusCell *cell = (MyOOrderStatusCell*)[tableView dequeueReusableCellWithIdentifier:OrderStatusCellIdentifier];
        if (cell == nil)
        {
            cell=[[MyOOrderStatusCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:OrderStatusCellIdentifier];
        }
    ...
    else if (row==[tableView numberOfRowsInSection:section]-1)
    {
        ...
        MyOOrderTotalCell *cell = (MyOOrderTotalCell*)[tableView dequeueReusableCellWithIdentifier:OrderTotalCellIdentifier];
        if (cell == nil)
        {
            cell=[[MyOOrderTotalCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:OrderTotalCellIdentifier];
        }
    ...
    {
        //int type=[[[_dataList objectAtIndex:section]objectForKey:@"orderType"]intValue];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0),
                   ^{
                       NSMutableDictionary *obj =[NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                  ...
                                                  [NSNumber numberWithInt:self.stat],@"stat",
                                                  nil];
                       ...
                       ASIFormDataRequest *request=[CommonMethods asiHttpRequestWithURL:loadOrderListByStat_URL andData:reqdata andDelegate:nil];
                       ...
                       if (root==nil){
                           ...
                           dispatch_async(dispatch_get_main_queue(),
                                          ^{
                                              ...
                                              NSArray *dataList=[data objectForKey:@"OrderList"];
